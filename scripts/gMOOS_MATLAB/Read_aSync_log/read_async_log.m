...
filename = 'L2_data_1_1_2010_____02_33_17.alog';
matFile = 'L2_data_1_1_2010_____02_33_17.mat';
tic
...
if use_pbar
    ...
    disp('Number of lines in the file are= ')
    disp(nLines)
...
%% Let's do this in peice meal

start_count = 0;

while curLine < start_count
    curLine=fgets(fid);
end
...
while ~feof(fid)%curLine<250000%~feof(fid)
    ...
    if strcmp('zSVID_1',message) && time > 568
        2+2
    end
    if isempty(val) %val will be empty if an array was attempted to be read in
        ...
        try
            m=textscan(val{1}(1:end),'%n','Delimiter',',');
        catch
...
for kk=2:length(dvc_names)
    if kk==2
        save(matFile, char(dvc_names{1,kk}),'-v7.3');
    else
        save(matFile, char(dvc_names{1,kk}),'-append','-v7.3');
    end
end

% also save start and end times of the log file
save(matFile, 'time','-append','-v7.3');
